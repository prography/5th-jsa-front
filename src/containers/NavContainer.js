...
const NavContainer = ({ history }) => {
  ...
  const OpenLoginDialog = useCallback(user => dispatch(openLoginDialog(user)), [
    dispatch,
  ]);
  const CloseLoginDialog = useCallback(
    user => dispatch(closeLoginDialog(user)),
    [dispatch],
  );
  const ShowSnackbar = useCallback(list => dispatch(showSnackbar(list)), [
    dispatch,
  ]);
  ...
  useEffect(() => {
    ...
    if (token) {
      api.getSigninCheck(token).then(res => {
  ...
  function onSuccess(result) {
    ...
    api.getKakaoSignin(result.response.access_token).then(res => {
