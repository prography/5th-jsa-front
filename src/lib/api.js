...
const getPizzaToppings = () => axios.get(`${apiServer}/pizzas/toppings`);

// large 토핑이미지 get
// 사용자가 drag and drop 등으로 토핑을 선택하면 피자 판 위에 올릴 토핑 이미지 값을 반환해줍니다.
const getPizzaToppingsImage = (topping) => axios.get(`${apiServer}/pizzas/toppings/image`, {
  params: {
    topping,
  },
});
...
// 결과 페이지에서 디테일 데이터 로드
const getPizzaDetail = (pk) => axios.get(`${apiServer}/pizzas/details/${pk}`);

// 랜덤 피자 조회
// 사용자가 랜덤 피자를 요청하면 랜덤으로 피자 데이터를 JSON형식으로 반환합니다.
const getPizzaRandom = () => axios.get(`${apiServer}/pizzas/random`);

// ------------------------------------------
// 3. feedback page api
// ------------------------------------------
const postFeedback = (content) => axios.post(`${apiServer}/users/feedback`, {
  content,
  headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
});
...
// 4. signin, signup api
// ------------------------------------------
const postSignup = (email, password, nickname) => axios.post(`${apiServer}/users/register`, {
  email,
  password,
  nickname,
  headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
const postSignin = (email, password) => axios.post(`${apiServer}/users/login`, {
  email,
  password,
  headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
});
...
const getKakaoSignin = (accessToken) => axios.get(`${apiServer}/users`, {
  headers: {
